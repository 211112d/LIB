...
local Services = setmetatable({}, {
    __index = LPH_NO_VIRTUALIZE(function(self, service, key)
        if Solara and (service == "VirtualInputManager") then
...
for Index, Value in Images do
    ...
    if not isfile(Location) then
        local ImageData = game:HttpGet("https://raw.githubusercontent.com/KingVonOBlockJoyce/imagessynex/main/"..Value)
        repeat wait() until ImageData ~= nil
        writefile(Location, ImageData)
    end
end
...
if not Mobile then
    ...
    function library:window(properties)
        ...
        local items = cfg.items; do
            ...
            local time_left = tostring((LRM_SecondsLeft < math.huge and ""..tostring(math.floor(((LRM_SecondsLeft / 60) / 60) / 24)).." days" or LRM_SecondsLeft == math.huge and "Freemium"))
    ...
    function library:colorpicker(options) 
        ...
        function cfg.set(color, alpha)
            ...
            local col = items[ "colorpicker" ].BackgroundColor3
            items[ "input" ].Text = string.format("%s, %s, %s, ", library:round(col.R * 255), library:round(col.G * 255), library:round(col.B * 255))
    ...
    function library:init_config(window) 
        ...
        column = main:column({})
        section = column:section({name = "Settings", side = "right", size = 1, default = false, icon = GetImage("Settings.png")})
        ...
        section:button({name = "Join Lowest Server", callback = function()
            ...
            if Server.id == game.JobId then
                library.notifications:create_notification({name = "KFChook", info = "You are currently in the smallest server!", lifetime = 10})
                return
            end 
